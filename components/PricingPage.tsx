...
const PricingPage: React.FC = () => {
  ...
  useEffect(() => {
    if (tiersInView && tiersContainerRef.current) {
      gsap.fromTo(".pricing-card-gsap", 
        { opacity: 0, y: 50, scale: 0.95 },
        { opacity: 1, y: 0, scale: (idx) => idx === 1 ? 1.05 : 1, duration: 1.2, stagger: 0.15, ease: "back.out(1.2)" }
      );
    }
  }, [tiersInView]);
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 pt-32 pb-20 transition-colors duration-500 overflow-hidden">
      ...
      <div ref={headerViewRef} className="max-w-7xl mx-auto px-4 text-center mb-16">
        <div ref={headerRef} className="space-y-6">
          ...
          <div className="mt-12 flex items-center justify-center gap-4">
            ...
            >
              <div className={`w-6 h-6 bg-blue-600 rounded-full transition-all transform ${billingCycle === 'annual' ? 'translate-x-8' : 'translate-x-0'} shadow-md`} />
            </button>
      ...
      {/* Pricing Cards */}
      <div ref={tiersViewRef} className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8 mb-32 relative">
        <div ref={tiersContainerRef} className="contents">
          {/* Starter */}
          <div className="pricing-card-gsap p-8 rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col hover:border-blue-400 transition-colors">
            ...
            <button className="w-full py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white rounded-xl font-bold hover:bg-slate-50 transition-all shadow-sm">Get Started</button>
          </div>

          {/* Pro */}
          <div className="pricing-card-gsap p-8 rounded-2xl border-2 border-blue-600 dark:border-blue-500 bg-white dark:bg-slate-900 shadow-2xl z-10 flex flex-col relative overflow-hidden group">
            ...
            <div className="absolute top-0 right-0 px-6 py-2 bg-blue-600 text-white text-[10px] font-bold uppercase tracking-[0.2em] rounded-bl-xl">Recommended</div>
            ...
            <button className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/20 active:scale-95">Upgrade Now</button>
          </div>
          ...
          <div className="pricing-card-gsap p-8 rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col hover:border-blue-400 transition-colors">
            ...
            <button className="w-full py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white rounded-xl font-bold hover:bg-slate-50 transition-all shadow-sm">Contact Sales</button>
          </div>
        </div>
      </div>
                  ...
                  {cat.features.map((f, fIdx) => (
                    <tr key={f.name} className={`matrix-row-gsap border-b border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors ${fIdx === cat.features.length - 1 ? 'border-b-2' : ''}`}>
                      ...
                      <td className="py-6 px-4 text-center text-sm font-bold text-slate-900 dark:text-white">
                        {typeof f.pro === 'boolean' ? (f.pro ? <CheckIcon className="w-5 h-5 text-blue-600 mx-auto" /> : <XMarkIcon className="w-5 h-5 text-slate-200 mx-auto" />) : f.pro}
                      </td>
                      <td className="py-6 px-4 text-center text-sm text-slate-500">
